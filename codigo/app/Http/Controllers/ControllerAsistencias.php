...
class ControllerAsistencias extends Controller
{
    ...
    {
        $user = Auth::user();
        $horario=horarios::where('id',$request->input('horario'))->where('user_id',$user->id)->first();
        ...
        if ($classroom) {
            $assistance = asistencias::where('user_id', $user->id)->where('aula_id', $classroom->id)->where('area_id', $areas->id)->where('horario_id',$request->input('horario'))->whereDate('fechaRegistro', $registrationdate)->first();
            if ($assistance) {
    ...
    public function update_assistance_data(Request $request, $id, $date)
    {
        ...
        foreach ($request->input('data') as $key => $val) {
            ...
            $Asistencia = asistencias::where('user_id', $user->id)->where('horario_id',$request->input('horario'))->whereDate('fechaRegistro', Carbon::parse($date))->where('aula_id', $classroom->id)->where('alumno_id', $objectstudent->id)->get();
    ...
    public function get_assistance(Request $request, $date, $idaula)
    {
        ...
        $object_students = asistencias::where('user_id', $user->id)->where('aula_id', $classroom->id)->where('horario_id',$request->input('horario'))->whereDate('fechaRegistro', Carbon::parse($date))->get();
        $dicc_students = [];
        if ($object_students) {
    ...
    public function generate_pdf(Request $request, $id)
    {
        ...
        $object_horario=horarios::where('id',$horario)->where('user_id',$user->id)->first();
        $object_area = areas::where('id', $object_horario->area_id)->where('user_id', $user->id)->first();
        $object_classroom = aulas::where('id', $object_horario->aula_id)->where('user_id', $user->id)->first();
        $object_student = alumnos::where('user_id', $user->id)->where(
            'id',
            $id
        )->where('aula_id', $object_classroom->id)->first();

        $object_asistance = asistencias::where('user_id', $user->id)->where('alumno_id', $object_student->id)->where('aula_id', $object_classroom->id)->where('area_id', $object_area->id)->where('horario_id',$object_horario->id)->whereDate('fechaRegistro', '>=', $fecha_inicio)->whereDate('fechaRegistro', '<=', $fecha_fin)->get();
        ...
        return view('asistencias/reportStudent', [
            ...
            'tardanzas' => $tardanzas,
            'asistances' => $lista,
            'horario'=>$object_horario->turno ." ".$object_horario->dia ." ".$object_horario->horainicio ." ".$object_horario->horafin, 
        ]);
    }
