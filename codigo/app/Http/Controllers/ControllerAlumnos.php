...
class ControllerAlumnos extends Controller
{
    ...
    public function students_all(Request $request)
    {
        ...
        foreach ($students2 as $key => $value) {
            ...
            $e = ['id' => $value->id, 'aula-turno'=>$s->turno,'apellidos' => $value->apellidos, 'nombres' => $value->nombres, 'grado' => $value->grado, 'seccion' => $value->seccion, 'aula-gr' => $s->grado, 'aula-se' => $s->seccion, 'aula-nivel' => $s->nivel, 'institucion' => $in->nombre];
            $students3[] = $e;
        }
