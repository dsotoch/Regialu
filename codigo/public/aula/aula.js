...
$(document).on('change', '#institucion', function () {
    ...
    if (nivel == '0') {
        error('Selecciona una Opción Valida');
...
$(document).on('click', '#btnsave', function (e) {
    ...
    try {
        $.ajax({
            ...
            data: { 'institucion': id_institu, 'seccion': seccion, 'grado': grado, 'nivel': nivel,'turno':turno, _token: csrftoken },
            ...
            success: function (response) {
                if (response.aula == 'error') {
                    ...
                    tabla.prepend('<tr> <td>' + institucion + '  </td><td>' + response.aula.grado + ' </td> <td>' + response.aula.seccion + '  </td><td>' + nivelo + ' </td><td>' + turno + ' </td> </tr>');
...
$(document).on('click', '.btn-edit', function () {
    ...
    $(document).on('click', '#editar', function () {
        ...
        var data = { 'grado': grado, 'seccion': seccion, 'nivel': nivel ,'turno':turno,_token:csrftoken};
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.response != 'error') {
                    ...
                    $(row.find('td')[6]).text(response.response.turno);
