...
$(document).on('change', '#institution', function () {
    ...
    if (value != 'Seleccione una Institución') {
        window.location.href = "/Alumnos/InstitucionSeleccionada/" + value + "/";
...
$(document).on('click', '#btn-save-alumno', function () {
    ...
    let data = { 'surnames': surnames, 'names': names, 'grade': grade, 'idClassroom': idClassroom ,_token:csrftoken};
    if (surnames == "" || names == "" || grade == "" || idClassroom=="0") {
...
$(document).on('click', '#btn-edit-institution', function () {
    ...
    let data = { 'names': nombres, 'surnames': apellidos,_token:csrftoken };

    $.ajax({
        type: "get",
        url: "/Alumnos/EditarAlumno/" + id + "/",
        data: data,
        dataType: "json",
