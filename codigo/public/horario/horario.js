...
$(document).on('change', '#turno', function () {
    ...
    if (nivel == '0') {
        error('Selecciona una Turno Valido');
...
$(document).on('change', '#dia', function () {
    ...
    if (nivel == '0') {
        error('Selecciona un Día Valido');
        $("#btnsave").attr("disabled", true);
...
$(document).on('change', '#area', function () {
    ...
    if (nivel == '0') {
        error('Selecciona una Area Valida');
        $("#btnsave").attr("disabled", true);
        ...
        if (turno == '0' || dia == '0' || area == '0') {
            ...
            try {
                $.ajax({
                    ...
                    data: { 'turno': turno, 'dia': dia, 'area': area, 'horainicio': horainicio, 'horafin': horafin, _token: csrftoken },
...
$(document).on('click', '.btn-delete', function (e) {
    ...
    swal({
        ...
        if (value == true) {
            $.ajax({
                type: "get",
                url: '/Horarios/EliminarHorario/' + id + '/',
            ...
            swal({
                ...
                if (value == true) {
                    $.ajax({
                        type: "get",
                        dataType: "json",
                        url: "/Horarios/ModificarHorario/" + id + "/",
                        data: { 'turno': turno, 'dia': dia, 'area': area, 'horainicio': horainicio, 'horafin': horafin, _token: csrftoken },
                        contentType: 'application/json; charset=utf-8',
                        success: function (response) {
                            if (response != 'error') {
...
$(document).on('click', '.btn-aula', function () {
    ...
    window.location.href = "/Horarios/AsignarAula/" + id + "/";
});
var horarios = {};
...
$(document).on('click', '#btn-elegido', function (e) {
    ...
    if (horarios.hasOwnProperty(id)) {
        error("Ya Elegiste Esa Aula...Escoge Otra!");
    } else {
...
$(document).on('click', '#btn-asignar', function () {
    ...
    if ((Object.keys(horarios).length == 0)) {
        error("No hay Aulas para Asignar...");
    } else {
        if ((Object.keys(horarios).length > 1)) {
            error("Error.Solo Debes de Elegir una Aula");
            ...
            $.ajax({
                ...
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                url: "/Horarios/AsignarDatosAula/" + id + "/",
                data: { 'data': horarios, _token: csrftoken },
                success: function (response) {
            ...
            $.ajax({
                ...
                success: function (response) {
                    if (response.response == 'success') {
                        ...
                        error("Hubo un problema al desligar ...Intentalo Nuevamente");
                    }
                }
            });
        }
