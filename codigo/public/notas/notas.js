...
$(document).on('click', '#btn-note', function () {
    ...
    $("#div-assistance").load('/Notas/IndexNotas');
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (response.value == 'horario') {
                ...
                if (response.value == 'area') {
                    error("No Existe Area Asociado al Horario Asignado al Aula");

                } else {
                    var ob = response;
                    $.each(ob, function (indexInArray, ob) {
                        schedule.append($("<option></option>").attr("value", ob.idHorario).text(ob.area + "/" + ob.dia + "/" + ob.horaInicio + "/" + ob.horaFin + "/" + ob.turno));
...
$(document).on('change', '#id_schedule', function () {
    ...
    if (area == '0') {
        ...
        error("Correcto..Ahora Selecciona un Tipo");
    }
...
$(document).on('change', '#unit', function () {
    ...
    if (unidad == '0') {
        error("Seleccione una Unidad Valida");
        table.clear().draw();
        ...
        let data = { 'unidad': unidad, 'tipo': guy + "-" + num, _token: csrftoken };
        $.ajax({
            type: "GET",
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            ...
            success: function (response) {
                ...
                if (student.length <= 0) {
                    error("No Existen Alumnos Registrados en esta Aula");
                } else {
                    $.each(student, function (key, student) {
                        let newrow = [student.idAlumno, student.apellidos, student.nombres, student.numeroNotas, '<div class="row"><div class="col-6"> <button class="btn btn-primary" id="btn-modal-note" title="Click para Asignar Notas"> <i class="fas fa-magic"></i>   </button> </div> <div class="col-6"> <button class="btn btn-info" id="btn-modal-note-all" title="Click para ver Todas las Notas"> <i class="fas fa-eye"></i>   </button>   </div>  </div>'];
...
function funguy(guy) {
    ...
    swal({
        ...
        if (res) {
            ...
            $("#num-guy").text(value);
            error("Correcto..Ahora Selecciona una Unidad");
...
$(document).on('click', '#save-note-modal', function () {
    ...
    let classroom = $("#id_schedule option:selected").val();
    ...
    if (description == "" || note == "") {
        ...
        $.ajax({
            ...
            data: {
                ...
                area: area,
                tipo: tipo + "-" + num,
                unidad: unidad, _token: csrftoken
            },
            success: function (response) {
...
$(document).on('click', '#btn-modal-note-all', function () {
    ...
    let area_hidden = $("#id_schedule option:selected").val();
    let parseArea = area.split("/")[0];
...
$(document).on('click', '#pdf-selected-student', function () {
    ...
    let areas = $("#id_schedule option:selected").val();
    ...
    let data = {
        ...
        alumnos: alumnos,
        periodo: periodo, _token: csrftoken
...
$(document).on('click', '#generate', function () {
    $.ajax({
        type: "get",
        url: "/Notas/Generate",
        data: { 'html': $('#pdf')[0].outerHTML, 'alumno': $("#alumno").text(), _token: csrftoken },

        success: function (data) {
            // Crear un enlace con el contenido devuelto
            var link = document.createElement('a');
            link.href = 'data:application/pdf;base64,' + data.pdf;
            link.download = data.alumno;
            link.target = '_blank';
